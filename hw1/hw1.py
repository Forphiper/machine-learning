...
Original file is located at
    https://colab.research.google.com/drive/1QSAfpgyxO7RjGaCFaNVyBtxWU6G9NbBp
...
def rfe_feat(train_data):
    ...
    return rfe.get_support(indices=True).tolist()

from sklearn.ensemble import RandomForestRegressor

def rf_feat(train_data, num_feat):
    ...
    return np.argpartition(model.feature_importances_, -num_feat)[-num_feat:].tolist()

high_corr_idx = []
for col in range(train_data.shape[1] - 1):
    if np.corrcoef(train_data[:, col], train_data[:, -1])[0][1] > 0.8:
        high_corr_idx.append(col)
...
def select_feat(train_data, valid_data, test_data, select_all=True):
    ...
    if select_all:
        ...
        feat_idx = rf_feat(train_data, 10)
        #feat_idx = high_corr_idx
