...
parser.add_argument("--seed", help="An integer input for random seed", type=int, default=2)
...
print(f"len of train questions= {len(train_questions)}")
dev_questions, dev_paragraphs = read_data("hw7_dev.json")
print(f"len of dev questions= {len(dev_questions)}")
...
class QA_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            paragraph_start = random.randint(answer_end_token - self.max_paragraph_len + 1, answer_start_token)
            paragraph_start = min(paragraph_start, len(tokenized_paragraph) - self.max_paragraph_len)
            paragraph_start = max(paragraph_start, 0)
            #print(f"answer_start_token= {answer_start_token} paragraph_start= {paragraph_start}")
...
def evaluate(data, oputput, doc_stride=doc_stride, paragraph=None, paragraph_tokenized=None):
    ##### TODO: Postprocessing #####
    ...
    for k in range(num_of_windows):
        ...
        if start_index > end_index:
            if start_prob >= end_prob:
                top_k_probs, top_k_indices = torch.topk(output.end_logits[k], k=50, dim=0)
                for i in range(len(top_k_indices)):
                    if start_index <= top_k_indices[i]:
                        end_prob = top_k_probs[i]
                        end_index = top_k_indices[i]
                        break
            else:
                top_k_probs, top_k_indices = torch.topk(output.start_logits[k], k=50, dim=0)
                for i in range(len(top_k_indices)):
                    if top_k_indices[i] <= end_index:
                        start_prob = top_k_probs[i]
                        start_index = top_k_indices[i]
                        break
        """
        # https://github.com/yaoweizhang/LHY2022-SPRING/blob/ebf0ec8ba17ef7193b2bebe25d677fc843b9d249/Hw07/answer/2022-hw7-boss.ipynb 
        token_type_id = data[1][0][k].detach().cpu().numpy()
        paragraph_start = np.argmax(token_type_id)
        paragraph_end = len(token_type_id) - np.argmax(token_type_id[::-1]) - 2
    ...
    # Check if there are characters that cannot be encoded
    # https://github.com/yaoweizhang/LHY2022-SPRING/blob/ebf0ec8ba17ef7193b2bebe25d677fc843b9d249/Hw07/answer/2022-hw7-boss.ipynb 
    if "[UNK]" in answer:
...
dev_loader = DataLoader(dev_set, batch_size=train_batch_size, shuffle=True, pin_memory=True)
test_loader = DataLoader(test_set, batch_size=1, shuffle=False, pin_memory=True)

from transformers import get_linear_schedule_with_warmup

total_steps = len(train_loader) + len(dev_loader)
...
#model = AutoModelForQuestionAnswering.from_pretrained(f"saved_model_seed2").to(device)
...
for epoch in range(num_epoch):
    ...
    for data in tqdm(dev_loader):
        ...
        if (step + 1) % logging_step == 0:
            print(f"Epoch {epoch + 1} | Step {step + 1} | loss = {train_loss.item() / logging_step:.3f}, acc = {train_acc / logging_step:.3f}")
            train_loss = train_acc = 0
    ...
    if validation:
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm(dev_loader)):
                ...
                eva = evaluate(data, output, doc_stride=doc_stride, 
                ...
                dev_acc += eva == dev_questions[i]["answer_text"]
...
print("Saving Model ...")
model_save_dir = f"saved_model_seed{seed}" 
...
from transformers.modeling_outputs import QuestionAnsweringModelOutput
models = []
for i in range(15, 16, 13): #81
    models.append(AutoModelForQuestionAnswering.from_pretrained(f"saved_model_seed{i}").to(device))
    models[(i-15)//13].eval()
"""
...
with torch.no_grad():
    for i, data in enumerate(tqdm(test_loader)):
        ...
        """
        # Ensemble the outputs from the models
        outputs = [model(input_ids=data[0].squeeze(dim=0).to(device), 
                         token_type_ids=data[1].squeeze(dim=0).to(device),
                         attention_mask=data[2].squeeze(dim=0).to(device)) 
                   for model in models]
        start_logits = torch.mean(torch.stack([output.start_logits for output in outputs]), dim=0)
        end_logits = torch.mean(torch.stack([output.end_logits for output in outputs]), dim=0)
        output = QuestionAnsweringModelOutput(start_logits=start_logits, end_logits=end_logits)
        """

        # https://github.com/yaoweizhang/LHY2022-SPRING/blob/ebf0ec8ba17ef7193b2bebe25d677fc843b9d249/Hw07/answer/2022-hw7-boss.ipynb 
        result.append(evaluate(data, output, doc_stride=doc_stride, 
...
prompts = ["根據文章，找出問題的答案\n",
    "According to the passage, print the answer of the question\n",
    "Selon le passage, imprimez la réponse à la question\n",
    "Imagine you're an information retrieval system that can answer user queries. Generate candidate answers based on the provided context\n",
    "假設文章為P，問題為Q，Q的答案為A，A為P的子集合，求A\n",
    "基於提供的文章內容，發揮判斷力，找出問題的確切解答\n",
    "Antwort auf die Frage gemäß dem Text geben\n"]

for j in range(len(prompts)):

    with open(f"in-context-learning-result{j}.txt", "w") as f:
