...
def mifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=20, decay=0.3):
    ...
    for i in range(num_iter):
        ...
        grad = x_adv.grad.detach()
        momentum = decay * momentum + grad.sign()
        x_adv = x_adv + alpha * momentum
...
def dim_mifgsm(model, x, y, loss_fn, epsilon=epsilon, alpha=alpha, num_iter=200, decay=1.0, sigma=1.0):
    x_adv = x.detach().clone().to(device)
    ...
    for i in range(num_iter):
        ...
        grad = x_adv.grad.detach()
        momentum = decay * momentum + grad.sign()
        random_noise = torch.randn_like(x_adv).to(device)
        random_noise = sigma * random_noise / torch.norm(random_noise)
        x_adv = x_adv + alpha * (momentum + random_noise)
...
def create_tar_file(dir_name):
    os.chdir(dir_name)  # Changing directory to 'fgsm'
    file_list = os.listdir('.')
    with tarfile.open(f'../{dir_name}.tgz', 'w:gz') as tar:
        for file in file_list:
            tar.add(file)
    os.chdir('..')  # Changing back to the previous directory
...
print(f"Ensemble {len(model_names)} models")
...
print(f'ensemble_fgsm_acc = {fgsm_acc:.5f}, ensemble_fgsm_loss = {fgsm_loss:.5f}')
create_dir(root, 'ensemble_fgsm', adv_examples, adv_names)
create_tar_file('ensemble_fgsm')
...
create_tar_file('ensemble_dim_mifgsm')
...
for i, cls_name in enumerate(classes):
    ...
    cnt += 1
    plt.subplot(len(classes), 4, cnt)
    im = Image.open(f'./dim_mifgsm/{path}')
    logit = ensemble_model(transform(im).unsqueeze(0).to(device))[0]
    predict = logit.argmax(-1).item()
    prob = logit.softmax(-1)[predict].item()
